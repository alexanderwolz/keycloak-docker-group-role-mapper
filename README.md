...
| ```REGISTRY_NAMESPACE_SCOPE```  | ```group```,```email```, ```username```  | If ```username``` is set, users will be granted full access to the namespace if it matches their username (lowercase check).<br>If ```email``` is set, users will be checked for their email address and will be granted access to the repository according their email domain.<br>If ```group``` is set, users will be checked for group membership and be granted access to the repository according their roles.<br>Namespace scope ```group``` is set by default or if value is empty or no value matches ```username```, ```email``` or ```group```. Values can be concatenated with ```,```. |
